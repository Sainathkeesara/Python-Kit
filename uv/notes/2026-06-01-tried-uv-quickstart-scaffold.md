...
Ran `uv init quickstart-demo` — it created a directory with `pyproject.toml`, `README.md`, and a `src/` layout with an `__init__.py` inside a package dir named after the project. No virtualenv yet, just a project skeleton.
...
I wrote a tiny script in the generated `main.py` inside the package dir:
...
Ran it with `uv run python` pointing at the generated `main.py`. It worked. The `uv run` part is nice — it activates the venv implicitly so I don't have to remember to `source .venv/bin/activate`.
...
- The generated `src/` layout with a package dir is fine but I'd rather have a flat `main.py`. I had to figure out the import path for my own script.
