...
- **Type narrowing** — A checker refining a variable's type inside a branch after a runtime check (e.g. `if x is not None:` narrows `Optional[str]` to `str`); ty and mypy differ in how far they push this inference.
...
- **PEP 561** — The standard for shipping type information inside a package: a `py.typed` marker file tells checkers the inline annotations are complete and safe to use downstream.
